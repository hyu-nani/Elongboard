...
void setup() {
	//Serial.begin(38400);
	BT_Serial.begin(38400); // Bluetooth baud 115200
	if(TEST==true){BT_Serial.println("N: -----Ready----- ");}
	setting_pin();
	if(TEST==true){BT_Serial.println("N: PinMode setting ");}
	neo_setting();
	if(TEST==true){BT_Serial.println("N: Neo setting ");}
	delay(100);
	if(TEST==true){BT_Serial.println("N: NEO pixel begin ");}
	Get_Data();
	if(TEST==true){BT_Serial.println("N: get data from EEPROM ");}
	GY_begin();
	if(TEST==true){BT_Serial.println("N: GY_87 begin ");}
		
	/* //bluetooth AT command setting motion
	BT_Serial.write("AT+BAUD6"); //BPS(1200=1, 2400=2, 4800=3, 9600=4, 19200=5, 38400=6, 57600=7, 115200=8)
	...
	*/
	...
	if(TEST==true){BT_Serial.println("N: BT AT set");}
	...
	if(TEST==true){BT_Serial.println("N: error data adjust");}
	ALL_TurnOff();
	if(TEST==true){BT_Serial.println("N: -----Start----- ");}
	initialize_neo =0;
	Default_Action();
	Compass();
	max_head = Head;
...
void loop() {
  ...
  //============================ charging mode ==============================
	...
	while(1){ //charging mode
		B_ALL_TurnOff();//하단부 OFF, 상단부배터리 표시
		...
		if(bat_count >= 20000){						
			...
			int i = 0;
			while(BT_Serial.available() > 0){ //buffer �Է�
				String_data[i] = BT_Serial.read();
				i++;
				if(i>string_buffer_size){
					break;
